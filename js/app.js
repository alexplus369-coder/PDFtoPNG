(() => {
            ...
            try {
                ...
                for (let i = 0; i < total; i++) {
                    ...
                    if (pageSize === 'original') {
                        ...
                        // Como jsPDF no soporta cambiar tamaño de página por página fácilmente,
                        // ajustamos la imagen al tamaño de página actual con márgenes.
                    }
    ...
    function initSplitPdf() {
        ...
        async function startSplitPdf() {
            ...
            try {
                ...
                // Cargar PDF fuente una sola vez
    ...
    function initRenameFiles() {
        ...
        async function startRenameFiles() {
            ...
            try {
                ...
                for (let i = 0; i < total; i++) {
                    ...
                    usedNames.add(name);
                    renameResults.push({ blob: item.file, name, url: item.url ? URL.createObjectURL(item.file) : null });
                }
        ...
        function showRenameResults() {
            ...
            renameResults.forEach((item, idx) => {
                ...
                const thumbHtml = isImage
                    ? `<img src="${item.url}" alt="${item.name}" class="page-thumb" loading="lazy">`
                    : `<div class="page-thumb" style="display:flex;align-items:center;justify-content:center;color:var(--error);">
                           <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="48" height="48">
                               <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                               <polyline points="14 2 14 8 20 8"/>
        ...
        function resetRenameMode() {
            ...
            renameResults.forEach(item => { if (item.url) URL.revokeObjectURL(item.url); });
            renameFiles = [];
            renameResults = [];
            isRenaming = false;
    ...
    // ============================================================
    // ============ INICIALIZACIÓN SEGURA DE CADA MODO =============
    // ============================================================
    try { initPdf2Img(); } catch (e) { console.error('Error al inicializar PDF a Imagen:', e); }
    try { initImg2Pdf(); } catch (e) { console.error('Error al inicializar Imagen a PDF:', e); }
    try { initMergePdf(); } catch (e) { console.error('Error al inicializar Unir PDF:', e); }
    try { initSplitPdf(); } catch (e) { console.error('Error al inicializar Dividir PDF:', e); }
    try { initRenameFiles(); } catch (e) { console.error('Error al inicializar Renombrar:', e); }
    try { initWord2Pdf(); } catch (e) { console.error('Error al inicializar Word a PDF:', e); }
    try { initPdf2Word(); } catch (e) { console.error('Error al inicializar PDF a Word:', e); }
})();
